...
describe('GET /slots', () => {
  ...
  it('returns slots for valid request with seeded availability', async () => {
    ...
    store.createAvailability(createTestAvailability({ startTime: '09:00', endTime: '10:00' }));
  ...
  it('returns empty array when a full-day exception blocks the date', async () => {
    ...
    store.createException(createTestException({ startTime: undefined, endTime: undefined }));
    const app = createApp(store);
