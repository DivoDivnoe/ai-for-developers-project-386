...
const mockStore = (overrides?: {
  ...
  listAvailability: vi.fn().mockReturnValue(overrides?.listAvailability ?? []),
...
describe('createBooking', () => {
  ...
  it('creates a booking with status confirmed', () => {
    const store = mockStore({
      listAvailability: [createTestAvailability({ startTime: '09:00', endTime: '10:00' })],
    });
  ...
  it('throws 409 when trying to book via exception without regular availability', () => {
    ...
    expect(() => createBooking(store, validInput)).toThrow('This time slot is not available');
  });

  it('throws 409 when booking in the past', () => {
    const store = mockStore();

    expect(() => createBooking(store, { ...validInput, startAt: PAST_DATE })).toThrow(
  ...
  it('throws 409 when time slot is not available', () => {
    const store = mockStore({
      listAvailability: [createTestAvailability({ dayOfWeek: 'tuesday' })],
    });
